...
//            System.out.println(redsNum + ", " + greensNum + ", " + bluesNum + " / " + denom);
        }
        return color;
    }
    ...
    private double[][] buildMask(Color[][] pixels, int k, int l, int maskSize) {
        ...
        for(int i = 0; i < maskSize; i++, k++) {
            ...
            for(int j = 0; j < maskSize; j++, l++) {
                double first_term = (Math.pow(k - y,2) + Math.pow(x - l, 2)) / powSdsDenom;
    ...
    private double getDiffSquared(Color[][] pixels, int k, int l, int y, int x){
        double r = Math.pow(pixels[k][l].getRed()-pixels[y][x].getRed(),2);
        double g = Math.pow(pixels[k][l].getGreen()-pixels[y][x].getGreen(),2);
        double b = Math.pow(pixels[k][l].getBlue()-pixels[y][x].getBlue(),2);
        return r + g + b;
    }
}
