...
public class DiffusorOperator {
    public static CustomImage anisotropicLorentz(CustomImage image, double lambda, int iterations, double sigma) {
        return anisotropic(image, lambda, iterations, sigma, new LorentzDetector(sigma));
    }

    public static CustomImage anisotropicLeclerc(CustomImage image, double lambda, int iterations, double sigma) {
        return anisotropic(image, lambda, iterations, sigma, new LeclercDetector(sigma));
    }

    public static CustomImage isotropicLeclerc(CustomImage image, double lambda, int iterations, double sigma) {
        return isotropic(image, lambda, iterations, sigma, new LeclercDetector(sigma));
    }
    ...
    private static CustomImage isotropic(CustomImage image, double lambda, int iterations, double sigma, EdgeDetector ed) {
        return difuse(new IsotropicDiffusion(), lambda, iterations, ed, image.getRGBRepresentation());
    }
    private static CustomImage anisotropic(CustomImage image, double lambda, int iterations, double sigma, EdgeDetector ed) {
